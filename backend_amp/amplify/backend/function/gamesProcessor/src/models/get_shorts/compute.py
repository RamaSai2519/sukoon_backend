...
class Compute:
    ...
    def _process_shorts(self, shorts):
        ...
        category = get_shorts_categories_collections().find_one(
        ...
        for short in shorts:
            ...
            counter += 1
            expert = get_experts_collections().find_one(
    ...
    def _get_shorts(self) -> dict:
        ...
        if not self.input.fetchedTill:
            ...
            for keyword in keywords:
                shorts = list(shorts_collection.find({"keyword": keyword, "approved": True}, {
                              "_id": 0, "thumbnails": 0, "description": 0}).sort({"createdAt": -1}).limit(3))
            ...
            for item in self.input.fetchedTill:
                ...
                shorts = list(shorts_collection.find({"keyword": keyword, "approved": True, "createdAt": {
                              "$gt": timestamp}}, {"_id": 0, "thumbnails": 0, "description": 0}).sort({"createdAt": -1}).limit(3))
                self._process_shorts(shorts)
