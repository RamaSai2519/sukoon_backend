...
class Compute:
    ...
    def _update_game_score(self, user_id, score) -> dict:
        ...
        if not user_stats_id:
            ...
            result = user_stats_collection.insert_one(
                {game_type_to__dictionary_key_mapping.get(self.input.game_type): game_stats})
            ...
            user_stats = user_stats_collection.find_one(
                {"_id": ObjectId(user_stats_id)})
            game_stats = user_stats.get(
                game_type_to__dictionary_key_mapping.get(self.input.game_type))
            ...
            if not game_stats:
                ...
                user_stats_collection.update_one(
                    {"_id": ObjectId(user_stats_id)},
                    {"$set": {game_type_to__dictionary_key_mapping.get(
                        self.input.game_type): game_stats}},
                )

    def compute(self):

        self._update_game_score(self.input.user_id, self.input.user_score)
        self._update_game_score(self.input.saarthi_id,
                                self.input.saarthi_score)
