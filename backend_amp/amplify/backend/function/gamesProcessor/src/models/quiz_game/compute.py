...
class Compute:
    ...
    def _get_questions_for_a_user_at_a_level(self, last_question_shown_at_this_level):
        ...
        total_questions_at_this_level = quiz_questions_collection.count_documents(
            {"level": self.input.level})
        if last_question_shown_at_this_level + self.input.question_to_show > total_questions_at_this_level:
            quiz_questions = list(quiz_questions_collection.find({"level": self.input.level, "category": self.input.category, "questionNumber": {
                                  "$gte": 1, "$lte": self.input.question_to_show}}, {"question": 1, "options": 1, "category": 1, "_id": 0}))
        else:
