...
from shared.db.referral import get_prcs_collection, get_prc_tracks_collection
...
class Compute:
    def __init__(self, input: Input) -> None:
        ...
        self.prcs_collection = get_prcs_collection()
        self.prc_tracks_collection = get_prc_tracks_collection()

    def validate_token(self) -> bool:
        query = {'token': self.input.token}
        token_doc = self.prcs_collection.find_one(query)
    ...
    def track_referral(self) -> ObjectId:
        ...
        inserted_id = self.prc_tracks_collection.insert_one(doc).inserted_id
        return inserted_id
